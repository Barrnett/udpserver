...
#include <memory.h>
#include <string.h>
#include "LocalControllerState.h"
...
CLocalControllerState::CLocalControllerState()
{
	Clear();
}
...
void CLocalControllerState::ClearResponse()
{
	//应答
...
bool CLocalControllerState::CheckCommand(char* data)
{
	if (!data)
	{
		return false;
	...
	//帧头标志HEAD
	MyMemMove(m_HEAD, data, sizeof(m_HEAD)-1);
...
bool CLocalControllerState::Response(int flowNumber)
{
	Clear();
	ClearResponse();
	...
	//帧命令类型 FT
	memset(m_FT, 0, sizeof(m_FT));
	YCodec::HexString2Bytes("0x81", m_FT, sizeof(m_FT)-1);
...
//获取通控器
bool CLocalControllerState::GetGECO(list<DEVICE> &lstDevs, string &strError)
{
...
//通控器ID
bool CLocalControllerState::AddGECOID(DEVICE device)
{
	memset(m_GECOID, 0 ,sizeof(m_GECOID));
...
//通控器名称
bool CLocalControllerState::AddGECONAME(DEVICE device)
{
	memset(m_GECONAME, 0, sizeof(m_GECONAME));
...
//设备编码
bool CLocalControllerState::AddEQCODING(DEVICE device)
{
	memset(m_EQCODING, 0, sizeof(m_EQCODING));
...
//硬件版本
bool CLocalControllerState::AddHARDWARE(DEVICE device)
{
	memset(m_HARDWARE, 0, sizeof(m_HARDWARE));
...
//软件版本
bool CLocalControllerState::AddSOFTWAR(DEVICE device)
{
	memset(m_SOFTWARE, 0, sizeof(m_SOFTWARE));
	int index = 0;
...
//网络IP地址
bool CLocalControllerState::AddNETWORKIP(DEVICE device)
{
	memset(m_NETWORKIP,0,sizeof(m_NETWORKIP));
...
//管理IP地址
bool CLocalControllerState::AddMANAGEMENTIP(DEVICE device)
{
	memset(m_MANAGEMENTIP,0,sizeof(m_MANAGEMENTIP));
...
//设备状态
bool CLocalControllerState::AddEQUIPMENTSTATE(DEVICE device)
{
	//设备状态
	memset(m_EQUIPMENTSTATE, 0, sizeof(m_EQUIPMENTSTATE));
...
//经度字符(度、分、秒)
bool CLocalControllerState::AddLONGITUDECHAR(DEVICE device)
{
	memset(m_LONGITUDECHAR, 0, sizeof(m_LONGITUDECHAR));
	//memset(m_LONGITUDEVALUE, 0, sizeof(m_LONGITUDEVALUE));
	memset(m_LONGITUDEDEGRR, 0, sizeof(m_LONGITUDEDEGRR));
...
//经度字符(度、分、秒) 
bool CLocalControllerState::AddLATITUDECHAR(DEVICE device)
{
	memset(m_LATITUDECHAR, 0, sizeof(m_LATITUDECHAR));
	//memset(m_LATITUDEVALUE, 0, sizeof(m_LATITUDEVALUE));
	memset(m_LATITUDEDEGRR, 0, sizeof(m_LATITUDEDEGRR));
...
//控制器级别
bool CLocalControllerState::AddCONTROLLERLEVEL(DEVICE device)
{
	memset(m_CONTROLLERLEVEL, 0, sizeof(m_CONTROLLERLEVEL));
...
void CLocalControllerState::charIntToByte(int i, char *bytes)
{
...
//拷贝函数
void* CLocalControllerState::MyMemMove(void *dst, const void *src, int count) 
{ 
	assert(dst); 
	assert(src); 
	void *ret=dst; 
